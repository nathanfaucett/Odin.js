...
define([
    ...
    function(Asset, Log) {
		...
		Assets.prototype.toSYNC = function(json) {
			...
			for (i = this.length; i--;) if ((jsonAsset = this[i]).sync) jsonAssets[i] = jsonAsset.toSYNC(jsonAssets[i]);
			return json;
		};
		...
		Assets.prototype.toJSON = function(json) {
			...
			for (i = this.length; i--;) if ((jsonAsset = this[i]).json) jsonAssets[i] = jsonAsset.toJSON(jsonAssets[i]);
			return json;
		};
