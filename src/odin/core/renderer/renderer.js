...
define([
    ...
    function(EventEmitter, Device, Dom, util, Mathf, Color, Vec3, Mat4, Enums, Log, TextureCube, ShaderChunks, Canvas) {
        ...
        function Renderer(opts) {
            ...
            this._lastTextureCube = undefined;
        ...
        Renderer.prototype.setUniform = function(location, value, key, type, isArray, index) {
            ...
            if (type === "int") {
                ...
                this.setTexture(location, value);
            } else if (type === "samplerCube") {
                this.setTextureCube(location, value);
            }
        };
        ...
        Renderer.prototype.setTexture = function(location, texture) {
            var gl = this.context,
                index = this._textureIndex++,
                glTexture;

            if (!texture || !texture.raw) {
                if (this._lastTexture !== this._whiteTexture) {
                    gl.activeTexture(gl.TEXTURE0 + index);
                    gl.bindTexture(gl.TEXTURE_2D, this._whiteTexture);
                    gl.uniform1i(location, index);
                    this._lastTexture = this._whiteTexture;
                }
                return;
            }
        ...
        Renderer.prototype.setTextureCube = function(location, cubeTexture) {
            ...
            if (!cubeTexture.needsUpdate && glTexture) {
                if (this._lastTextureCube !== glTexture) {
                    gl.activeTexture(gl.TEXTURE0 + index);
                    gl.bindTexture(gl.TEXTURE_CUBE_MAP, glTexture);
                    gl.uniform1i(location, index);
                    this._lastTextureCube = glTexture;
                }
                return;
            }
        ...
        Renderer.prototype._bindCustomUniforms = function(customUniforms, uniforms, materialName, materialUniforms) {
            ...
            while (i--) {
                ...
                if (!value && customUniform.type !== "sampler2D") throw "WebGLRenderer bindShader: material " + materialName + " was not given a uniform named " + name;
